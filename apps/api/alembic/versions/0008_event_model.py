...
def upgrade() -> None:
    # Create events table with analyst-driven incident tracking.
    op.create_table(
        "events",
        ...
        sa.Column("severity", sa.String(1), nullable=False, server_default="C"),
    ...
    op.create_index("ix_events_created_at", "events", ["created_at"], postgresql_desc=True)
    op.create_index("ix_events_created_by", "events", ["created_by"])
    op.create_index("ix_events_occurred_at", "events", ["occurred_at"], postgresql_desc=True)

    # Constraints for status, severity, and confidence.
    op.create_check_constraint(
        "ck_events_status", "events",
        "status IN ('pending','confirmed','dismissed','closed')"
    )
    op.create_check_constraint(
        "ck_events_severity", "events",
        "severity IN ('A','B','C','D','E','F')"
    )
    op.create_check_constraint(
        "ck_events_confidence", "events",
        "confidence >= 0.0 AND confidence <= 1.0"
    )

    # Create event_archive join table (analyst-driven archive links).
    op.create_table(
        "event_archive",
        sa.Column("event_id", postgresql.UUID(as_uuid=True), nullable=False, primary_key=True),
        sa.Column("archive_id", postgresql.UUID(as_uuid=True), nullable=False, primary_key=True),
    ...
    op.create_index("ix_event_archive_linked_at", "event_archive", ["linked_at"], postgresql_desc=True)
    op.create_index("ix_event_archive_linked_by", "event_archive", ["linked_by"])
    op.create_index("ix_event_archive_archive_id", "event_archive", ["archive_id"])
