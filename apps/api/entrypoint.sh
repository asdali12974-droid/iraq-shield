...
run_migrations() { echo "[entrypoint] alembic upgrade head"; alembic upgrade head; }
run_bootstrap()  { echo "[entrypoint] seeding RBAC + admin"; python -m app.modules.iam.bootstrap; }

case "${1:-serve}" in
  serve)
    wait_for_db; run_migrations; run_bootstrap
    echo "[entrypoint] starting API"
    exec uvicorn app.main:app --host 0.0.0.0 --port 8000 --proxy-headers
    ;;
  migrate)   wait_for_db; run_migrations ;;
  bootstrap) wait_for_db; run_bootstrap ;;
  worker)
    wait_for_db
